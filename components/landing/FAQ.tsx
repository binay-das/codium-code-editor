        ...
        <section className="py-24 bg-white dark:bg-black">
            <div className="lg:w-5xl w-full mx-auto px-6">
                <div className="text-center mb-16 max-w-3xl mx-auto">
                    ...
                    <p className="text-gray-600 dark:text-zinc-400">Everything you need to know about Codium</p>
                </div>
                ...
                <div className="w-full max-w-3xl mx-auto space-y-4">
                    {faqs.map((faq, index) => (
                        <div
                            key={index}
                            className="border border-gray-200 dark:border-zinc-800 rounded-lg bg-gray-50 dark:bg-zinc-900/30 overflow-hidden w-full"
                        >
                            <button
                                onClick={() => setOpenIndex(openIndex === index ? null : index)}
                                className="w-full flex items-center justify-between p-6 text-left hover:bg-gray-100 dark:hover:bg-zinc-900/50 transition-colors focus:outline-none"
                            >
                                <span className="text-lg font-medium text-gray-900 dark:text-white">{faq.question}</span>
                                <ChevronDown
                                    className={`h-5 w-5 text-gray-500 dark:text-zinc-500 transition-transform duration-300 shrink-0 ${openIndex === index ? "rotate-180" : ""
                                        }`}
                                />
                            </button>
                            ...
                            <AnimatePresence initial={false}>
                                {openIndex === index && (
                                    ...
                                    >
                                        <div className="p-6 pt-0 text-gray-600 dark:text-zinc-400 leading-relaxed border-t border-gray-200 dark:border-zinc-800/50">
                                            {faq.answer}
                                        </div>
                                    </motion.div>
                                )}
